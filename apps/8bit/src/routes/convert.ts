...
import { createOpenAIClient, convertTo8Bit } from "../libs/openai";
    ...
    try {
      ...
      if (!targetPath || targetPath.trim() === "") {
        return res.status(400).send(`
          <!DOCTYPE html>
          <html>
          <head><title>8-Bit Converter - Error</title></head>
          <body>
            <h1>Error: No URL provided</h1>
            <p>Please provide a URL in the path. Example: /${encodeURIComponent("https://example.com")}</p>
          </body>
          </html>
        `);
      }
      ...
      log.info(`Converting ${targetUrl} to 8-bit...`);
      ...
      // Create OpenAI client and convert
      const openaiClient = createOpenAIClient({
        apiKey: config.openai.apiKey,
        model: config.openai.model,
      });

      const eightBitHtml = await convertTo8Bit(
        openaiClient,
        {
          targetUrl,
          maxTokens: 16000,
      ...
      log.info(`Successfully converted ${targetUrl} to 8-bit`);

      // Set appropriate content type and send the HTML
      res.setHeader("Content-Type", "text/html; charset=utf-8");
      res.send(eightBitHtml);
    } catch (err: any) {
