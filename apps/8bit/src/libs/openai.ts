...
export interface IEightBitConversionRequest {
  targetUrl: string;
  maxTokens?: number;
}
...
export async function convertTo8Bit(
  client: OpenAI,
  request: IEightBitConversionRequest,
  ...
  const userPrompt = `Convert the following website into a complete, self-contained 8-bit styled HTML document.
