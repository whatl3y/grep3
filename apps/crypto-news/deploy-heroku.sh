...
# Crypto News - Heroku Container Deployment Script
#
# This script deploys the crypto news app to Heroku using Docker containers:
...
#
# If APP_NAME is not provided, it will use 'grep3-news' as default.

APP_NAME="${1:-grep3-news}"
...
build_images() {
    ...
    if [ "$DEPLOY_SCHEDULER" = true ]; then
        ...
        docker buildx build --platform linux/amd64 \
            -f apps/crypto-news/Dockerfile.scheduler \
    ...
    if [ "$DEPLOY_WORKER" = true ]; then
        ...
        docker buildx build --platform linux/amd64 \
            -f apps/crypto-news/Dockerfile.worker \
...
deploy() {
    ...
    echo "  heroku config:set DATABASE_URL=<your-postgres-url> -a $APP_NAME"
    echo "  heroku config:set REDIS_URL=<your-redis-url> -a $APP_NAME"
    echo "  heroku config:set OPENAI_API_KEY=<your-key> -a $APP_NAME"
    echo "  heroku config:set CRYPTOPANIC_API_TOKEN=<your-token> -a $APP_NAME"
    echo ""
}
