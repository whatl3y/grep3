...
# Execution Engine - Heroku Container Deployment Script
#
# This script deploys the execution-engine application to Heroku using Docker containers:
...
#
# If APP_NAME is not provided, it will use 'grep3-execution-engine' as default.

APP_NAME="${1:-grep3-execution-engine}"

# Check if first arg is an option (starts with --)
if [[ "$APP_NAME" == --* ]]; then
    APP_NAME="grep3-execution-engine"
fi
...
build_images() {
    ...
    if [ "$DEPLOY_WEB" = true ]; then
        ...
        docker buildx build --platform linux/amd64 \
            -f apps/execution-engine/Dockerfile \
    ...
    if [ "$DEPLOY_SCHEDULER" = true ]; then
        ...
        docker buildx build --platform linux/amd64 \
            -f apps/execution-engine/Dockerfile.scheduler \
    ...
    if [ "$DEPLOY_WORKER" = true ]; then
        ...
        docker buildx build --platform linux/amd64 \
            -f apps/execution-engine/Dockerfile.worker \
