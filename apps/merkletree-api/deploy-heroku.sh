...
# Merkle Tree API - Heroku Container Deployment Script
#
# This script deploys the merkletree-api application to Heroku using Docker containers:
...
#
# If APP_NAME is not provided, it will use 'grep3-merkletree-api' as default.

APP_NAME="${1:-grep3-merkletree-api}"

# Check if first arg is an option (starts with --)
if [[ "$APP_NAME" == --* ]]; then
    APP_NAME="grep3-merkletree-api"
fi
...
build_images() {
    ...
    if [ "$DEPLOY_WEB" = true ]; then
        ...
        docker buildx build --platform linux/amd64 \
            -f apps/merkletree-api/Dockerfile \
    ...
    if [ "$DEPLOY_SCHEDULER" = true ]; then
        ...
        docker buildx build --platform linux/amd64 \
            -f apps/merkletree-api/Dockerfile.scheduler \
    ...
    if [ "$DEPLOY_WORKER" = true ]; then
        ...
        docker buildx build --platform linux/amd64 \
            -f apps/merkletree-api/Dockerfile.worker \
